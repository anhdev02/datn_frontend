...
const BestSeller = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/product/bestseller/36")
      .then((res) => res.json())
  ...
  const currentItems = productsBestSeller.slice(indexOfFirstItem, indexOfLastItem);

  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(productsBestSeller.length / itemsPerPage); i++) {
    pageNumbers.push(i);
  }
